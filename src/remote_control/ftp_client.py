...
class Launcher(metaclass=abc.ABCMeta):
    ...
    def launch_linux(self, command_line):
        sp = subprocess.Popen('cd {0}; python3 {1} > {2}'.format(self.path_to_target, command_line, self.logs),
                              shell=True,
                              executable='/bin/bash')
        sp.communicate()
...
def cli_parser():
    ...
    parser.add_argument('-p', '--password',
                        ...
                        required=True)
    parser.add_argument('-b', '--benchmark_config',
                        type=str,
                        help='Path to benchmark config file.',
                        required=False)
    parser.add_argument('-ac', '--accuracy_checker_config',
                        type=str,
                        help='Path to accuracy checker config file.',
                        required=False)
    parser.add_argument('--benchmark_executor',
                        type=str,
                        help='Type of benchmark executor.',
                        required=False)
    parser.add_argument('--accuracy_checker_executor',
                        type=str,
                        help='Type of accuracy checker executor.',
                        required=False)
    parser.add_argument('-os', '--os_type',
                        type=str,
                        help='Type of operating system.',
                        required=True)
    parser.add_argument('--ftp_dir',
                        type=str,
                        help='Path to the directory with results on the FTP.',
                        required=True)
    parser.add_argument('--benchmark_res_file',
                        type=str,
                        help='The name of the file to which benchmark results are written',
                        required=False)
    parser.add_argument('--accuracy_checker_res_file',
                        type=str,
                        help='The name of the file to which accuracy checker results are written',
                        required=False)
    parser.add_argument('--benchmark_log_file',
                        type=str,
                        help='The name of the file to which benchmark logs are written',
                        required=False)
    parser.add_argument('--accuracy_checker_log_file',
                        type=str,
                        help='The name of the file to which accuracy checker logs are written',
                        required=False)
    parser.add_argument('--accuracy_checker_source',
                        help='Path to directory in which input images will be searched',
                        type=str,
                        required=False)
    parser.add_argument('--accuracy_checker_definitions',
...
def main():
    ...
    if param_list.benchmark_config:
        benchmark = BenchmarkLauncher(path_to_ftp_client, param_list.benchmark_config, param_list.benchmark_executor,
    ...
    if param_list.accuracy_checker_config:
        accuracy_checker = AccuracyCheckerLauncher(path_to_ftp_client, param_list.accuracy_checker_config,
