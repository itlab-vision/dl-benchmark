...
- Набор тегов для тестирования вывода средствами Intel Distribution
  ...
  - 'ChangePreprocessOptions'- тег, необязательный для заполнения; только для случая, когда `frontend` не равен `ir`
    ...
    Command line может отличаться для разных версий benchmark_app (Дает возможность переименования
    заданных опций препроцессинга).
...
- Набор тегов для тестирования вывода средствами ONNX Runtime, OpenCV DNN CPP, PyTorch CPP:
