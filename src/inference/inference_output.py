...
def classification_output(result, labels, number_top, log):
    ...
    image_count = 1
    for res in result:
        for i, probs in enumerate(res['prob']):
...
def segmentation_output(result, color_map, log):
    ...
    image_count = 1
    for res in result:
        for batch, data in enumerate(res['prob']):
...
def infer_output(result, input, labels, number_top, prob_threshold,
    ...
    if task == 'feedforward' or len(result) == 0:
        ...
        classification_output(result, labels, number_top, log)
    elif task == 'detection':
        detection_output(result, input['data'], prob_threshold)
    elif task == 'segmentation':
        segmentation_output(result, color_map, log)