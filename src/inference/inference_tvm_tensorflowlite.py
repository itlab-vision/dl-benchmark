...
class TensorFlowLiteToTVMConverter(Converter):
    ...
    def _convert_model_from_framework(self, target, dev):
        ...
        print(self.args['input_shape'])
        shape_dict = {self.args['input_name']: (1,224,224,3)}
        dtype = {self.args['input_name']: 'float32'}
        log.info('Creating graph module from TensorFlow model')
        model, params = tvm.relay.frontend.from_tflite(model_tf, shape_dict=shape_dict, dtype_dict=dtype)
        return model, params
...
def main():
    ...
    try:
        ...
        converter = TensorFlowLiteToTVMConverter(create_dict_for_converter_tensorflow(args))
