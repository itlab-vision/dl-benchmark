...
- `-i / --input` - путь до графа или (расширение `.bin`).
...
- `-t / --task` - наименование решаемой задачи (`node-classification`).
  По умолчанию производится приведение к виду таблицы.
- `--raw_output` - работа скрипта без логов. По умолчанию не установлен.
...
Результат выполнения: таблица, сопоставляющая вершинам вероятности принадлежности к классам.
