...
## Вывод глубоких моделей с использованием PaddlePaddle
...
```bash
inference_paddlepaddle.py
```

Обязательные аргументы:

- `-m / --model` - путь до pdmodel-файла с обученной глубокой моделью.
- `-p / --params` - путь до pdiparams-файла с весами.
- `-i / --input` - путь до изображения или директории с изображениями
  (расширения файлов `.jpg`, `.png`, `.bmp` и т.д.).
...
- `-nthreads / --number_threads` - максимальное число потоков для исполнения вывода.
  По умолчанию будет использоваться количество потоков, равное числу физических
  ядер в системе.
- `--memory_pool_init_size_mb` - количество памяти внутри GPU, выделенное под вывод.
  Не имеет эффекта при выводе на CPU.
...
```bash
python3  python inference_paddlepaddle.py \
    -m <path_to_model>\<model_name>.pdmodel -p <path_to_model_parameters>\<model_parameters>.pdiparams \
    -t classification -i <path_to_image>/<image_name>
```
...
- `-ni / --number_iter` - количество прямых проходов по сети.
  По умолчанию выполняется один проход по сети.
- `--high_level_ir` - высокоуровневое представление: `relax` или `relay`. По умолчанию `relay`.
- `-vm / --virtual_machine` - использование Virtual Machine API. По умолчанию не установлен.
