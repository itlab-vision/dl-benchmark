...
from config_parser import NNCFQuantizationConfigParser
from model_readers import NNCFModelReader
from parameters import NNCFDatasetReader, NNCFQuantParamReader, NNCFQuantizationProcess
...
def main():
    ...
    try:
        parser = NNCFQuantizationConfigParser(args.config)
        data_reader = NNCFDatasetReader(log)
        quant_params = NNCFQuantParamReader(log)
        model_reader = NNCFModelReader.get_reader('onnx', log=log)
