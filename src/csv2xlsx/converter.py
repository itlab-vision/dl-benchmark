...
def build_parser():
    ...
    parser.add_argument('-t', '--table', type=str, \
                        help='Path to the inference table in csv format.', \
                        nargs='+', required=True)
    ...
    return path_table_csv, path_table_xlsx, table_kind


def convert_csv_table_to_xlsx(path_table_csv, path_table_xlsx, table_type):
    if table_type == 'benchmark':
        table_xlsx = XlsxBenchmarkTable(path_table_csv, path_table_xlsx)
...
def main():
    path_table_csv, path_table_xlsx, table_type = build_parser()
    convert_csv_table_to_xlsx(path_table_csv, path_table_xlsx, table_type)
