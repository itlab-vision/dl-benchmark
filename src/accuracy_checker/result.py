class result:
    def __init__(self, status, model, launcher, device, dataset, precision, accuracy):
    ...
    def get_result_dict(self):
        return {'status': self.__status, 'model': self.__model, 'launcher': self.__launcher, 'device': self.__device,
                'dataset': self.__dataset, 'precision': self.__precision, 'accuracy': self.__accuracy}
    ...
    def parser_test_result(res, idx):
        ...
        precisions = [value[len(TAG_PRECISION):].replace('\r', '') for value in res if TAG_PRECISION in value] \
            if not error else ['']
        if not precisions:
            precisions = ['FP32']
        if not error:
        ...
        return [result(status, models[0], launchers[0], devices[0], datasets[0], precisions[0], accuracy)
