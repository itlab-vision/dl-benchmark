...
class HTMLAccuracyCheckerTable(HTMLTable):
    ...
    def _get_column_dict(self):
        ...
        for framework in self._frameworks_list:
            if framework['name'] == 'OpenVINO DLDT':
                dlsdk = framework
                break
        for row_index in range(1, len(self._table_csv)):
            try:
                device_dict[self._table_csv[row_index][DEVICE_POSITION_IN_TABLE]]
            except KeyError:
                if self._table_csv[row_index][DEVICE_POSITION_IN_TABLE] in dlsdk:
                    device_dict[self._table_csv[row_index][DEVICE_POSITION_IN_TABLE]] = \
                        dlsdk[self._table_csv[row_index][DEVICE_POSITION_IN_TABLE]].replace(' ', '').split(',')
        return device_dict

    def _added_all_test(self, models_dict):
        for plugin in self._column_dict:
            weights = self._column_dict[plugin]
            self._column_dict[plugin] = {weight: {} for weight in weights}
            for weight in weights:
                for model in models_dict:
                    self._column_dict[plugin][weight][model] = {}
                    for type in models_dict[model]['accuracy_type']:
                        self._column_dict[plugin][weight][model][type] = self.__find_test(model, plugin, type, weight)

    def __find_test(self, model, plugin, accuracy_type, precision):
        for row_index in range(1, len(self._table_csv)):
    ...
    def create_table_header(self):
        ...
        for plugin in self._column_dict:
            ...
            for weights in self._column_dict[plugin]:
                self._table_html.append('<th><table align="center" width="100%" border="1" cellspacing="0" cellpadding="0" class="standard_table">\n')
        ...
        for task_type in self._task_types_dict:
            ...
            for plugin in self._column_dict:
                ...
                for model in self._task_types_dict[task_type]:
                    ...
                    for weight in self._column_dict[plugin]:
                        self._table_html.append('<td height="120px"><table align="center" class="result_column" border="1" cellspacing="0" cellpadding="0" class="standard_table">')
                        for accuracy_type in self._task_types_dict[task_type][model]['accuracy_type']:
                            self._table_html.append('\n<tr><td><table align="center" border="1" cellspacing="0" cellpadding="0" class="standard_table">\n')
                            self._table_html.append('<tr><td class="double" align="right">{}</td>\n</tr>'.format(
                                self._column_dict[plugin][weight][model][accuracy_type]))
                            self._table_html.append('\n</table></td>\n</tr>')
                        self._table_html.append('</table>\n</td>')
                    self._table_html.append('</table>\n</td>\n</tr>')
                self._table_html.append('</table>\n</td>')
