...
class HTMLTable:
    ...
    def find_plugin_in_infr(self, plugin, target_infr):
        for row_index in range(1, len(self.table_csv)):
            if (plugin in self.table_csv[row_index][PARAMS_POSITION_IN_TABLE] and target_infr == self.table_csv[row_index][INFR_POSITION_IN_TABLE]):
                return True
        return False
    ...
    def create_table_header(self):
        ...
        for infrastr in self.infr_dict:
            self.table_html.append('<th> <table align="center" width="100%" border="1" cellspacing="0" cellpadding="0" class="standard_table">\n')
            ...
            for framework in self.infr_dict[infrastr]:
                self.table_html.append('<th> <table align="center" width="100%" border="1" cellspacing="0" cellpadding="0" class="{}">\n<tr>')
                self.table_html.append('\n<th>{}</th></tr><tr>'.format(framework))
                self.table_html.append('<td> <table align="center" width="100%" border="1" cellspacing="0" cellpadding="0" class="standard_table">\n<tr>')
                for plugin in self.infr_dict[infrastr][framework]:
                    supported_mode = HTMLTable.get_supported_mode(plugin)
                    table_header_class = "standard_table" if (supported_mode == 'ALL' and len(self.infr_dict[infrastr][framework]) > 1) else "one_type_table"
                    self.table_html.append('<th> <table align="center" width="100%" border="1" cellspacing="0" cellpadding="0" class="{}">\n<tr>'.format(table_header_class))
                    self.table_html.append('\n<th>{}</th>\n</tr>\n<tr>\n'.format(plugin))
                    self.table_html.append('<td> <table align="center" width="100%" border="1" cellspacing="0" cellpadding="0" class="{}">\n<tr>'.format(table_header_class))
    ...
    def write_test_results(self):
        ...
        for task_type in self.task_types_dict:
            ...
            # Print models
            self.table_html.append('<td> <table align="center" class="lock" height="100%" border="1" cellspacing="0" cellpadding="0" class="standard_table">\n')
            ...
            # Print batch
            self.table_html.append('<td > <table align="center" class="lock" border="1" cellspacing="0" cellpadding="0" class="standard_table">\n')
            for model in self.task_types_dict[task_type]:
            ...
            for infrastr in self.infr_dict:
                ...
                for model in self.task_types_dict[task_type]:
                    ...
                    for framework in self.infr_dict[infrastr]:
                        for plugin in self.infr_dict[infrastr][framework]:
                            for weight in self.infr_dict[infrastr][framework][plugin]:
                                self.table_html.append('<td height="120px"> <table align="center" class="result_column" border="1" cellspacing="0" cellpadding="0" class="standard_table">')
                                for batch in self.task_types_dict[task_type][model]['batch']:
                                    supported_mode = HTMLTable.get_supported_mode(plugin)
                                    table_class = "standard_table" if supported_mode == 'ALL' else "one_type_table"
