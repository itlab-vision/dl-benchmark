...
на удаленных вычислительных узлах в Docker-контейнерах. В примере используется OpenVINO 2022.1.
...
1. Подготовить Docker-образ системы, который будет удаленно развернут на необходимых инфраструктурах.
Для этого необходимо перейти в директорию с Docker-файлами и собрать необходимый образ,
используя инструкцию, которая находится в соответствующей директории [docker][docker].
...
4. Запустить модуль автоматического развертывания системы, используя [руководство][config].
   ...
   Скрипт копирует архив с Docker-образом на FTP-сервер, после чего уже с FTP-сервера скрипт копирует его на все удаленные узлы,
...
клонирование которого происходит внутри развертывания Docker-образов в директорию `/tmp`.
...
Заметим, что в конфигурационных файлах пути прописываем относительно работы внутри Docker-контейнера. Сами конфигурационные файлы мы сохраняем
и копируем на FTP-сервер, с которого скрипт в свою очередь потом их перенесет на соответствующие удаленные машины. Пусть в нашем примере
...
путь до модели указывается относительно окружения внутри Docker-контейнера, а путь до конфигурационного
...
Конфигурационный файл мы сохраняем и копируем на FTP-сервер, с которого скрипт впоследствии в свою очередь
...
и сохранить его по адресу `/home/itmm/ftp/remote/`.
...
2. Запустить эксперименты удаленно. Для этого необходимо воспользоваться утилитой `screen` для Linux, которая позволяет создавать фоновые процессы. В результате ее использования можно запустить скрипт с массовыми экспериментами, а после выключить терминал.
   ...
   В результате выполнения указанной команды создается новое окно `validation`, в котором мы уже можем смело работать.
